...
def RK2():
    ...
    for k in range(N):
        ...
        dS1 = -(1 - u) * beta * S_k * I_k
        dI1 = (1 - u) * beta * S_k * I_k - nu * I_k
        dR1 = nu * I_k
        ...
        dS2 = -(1 - u) * beta * S_mid * I_mid
        dI2 = (1 - u) * beta * S_mid * I_mid - nu * I_mid
        dR2 = nu * I_mid
    ...
    sirRK2.minimize((S[-1] - S_star)**2) if fcosto==1 else sirRK2.minimize((S[-1] - S_star)**2 + u**2)
    ...
    solRK2 = sirRK2.solve()
    print(solRK2.value(u))
    # print(solRK2.value(I[-1]))

    tiempos = np.linspace(desde, desde+cant_dias, N+1)
    plt.subplot(211)
    plt.plot([0,desde],[0,0], color='navy')
    plt.plot(tiempos,[solRK2.value(u)]*len(tiempos),label=r'$u(t)$',color='navy', drawstyle='default')
    ...
    plt.grid()
    plt.legend()
    plt.savefig(f'graficos/constante{fcosto}.svg') if guardar else None
    ...
    if planoSI:
        ...
        plt.savefig(f'graficos/SIconstante{fcosto}.svg') if guardar else None
        plt.show() if graficas else None
        plt.clf()
