...
def RK2():
    ...
    sirRK2.minimize((S[-1] - S_star)**2+ sumsqr(u)+I[-1])    
    # Resolver
    p_opts = {}
    ...
    plt.grid()
    plt.legend()
    plt.savefig(f'graficos/optima{fcosto}.svg') if guardar else None
    ...
    if planoSI:
        ...
        plt.savefig(f'graficos/SIoptima{fcosto}.svg') if guardar else None
        plt.show() if graficas else None
        plt.clf()
